...
app.get("/booking/:pk", (req, res) => {
    ...
    let myQuery = `SELECT * FROM Guide
    LEFT JOIN Booking
    ON Booking.GuideFK = Guide.GuidePK
    where GuidePK = ${pk}`
    ...
    .catch((err)=>{
        console.log("error in /booking/:pk", err);
        res.status(500).send();
    });
});

app.post("/contacts/logout", async (req, res)=>{
    // console.log("/contacts/login called ", req.body)
